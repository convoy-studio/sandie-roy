...
define(["ParticlesGroup", "Movieclip"], function(ParticlesGroup, Movieclip) {
  ...
  MouseFollower = (function(_super) {
    ...
    MouseFollower.prototype.onParticleCreated = function(p) {
      ...
      target = p.target;
      if (target) {
        this.particlesGroup.emitterpos.x += (Dvorak.Mouse.projection.x - this.particlesGroup.emitterpos.x) * 0.1;
        this.particlesGroup.emitterpos.y += (Dvorak.Mouse.projection.y - this.particlesGroup.emitterpos.y) * 0.1;
        this.particlesGroup.emitterpos.y += 10.8;
        this.particlesGroup.emitterpos.z += (Dvorak.Mouse.projection.z - this.particlesGroup.emitterpos.z) * 0.1;
        this.particlesGroup.values_color[target].setHex(this.particleColor[0]);
        this.particlesGroup.values_size[target] = Math.random() * 100 + (Sound.frequency / 200) * 400;
